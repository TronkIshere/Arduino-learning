...
void loop() {
  // Gọi hàm random để chọn một hành động
  int anim = random(0, 9); // Thay đổi giới hạn trên nếu bạn thêm nhiều hành động

  // Thực hiện hành động dựa trên giá trị ngẫu nhiên
  switch(anim) {
    case 0:
      wakeup();
  ...
  delay(1000); // Delay trước khi lặp lại hành động
}
